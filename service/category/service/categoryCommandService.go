...
func NewCategoryCommandService(deps *CategoryCommandServiceDeps) *categoryCommandService {
...
func (s *categoryCommandService) Create(ctx context.Context, req *requests.CreateCategoryRequest) (*models.Category, error) {
	const method = "Create"
	...
	if err != nil {
		status = "error"
		return errorhandler.HandleError[*models.Category](s.logger, err, method, span, zap.Any("request", req))
	}
...
func (s *categoryCommandService) Update(ctx context.Context, req *requests.UpdateCategoryRequest) (*models.Category, error) {
	const method = "Update"

	ctx, span, end, status, logSuccess := s.observability.StartTracingAndLogging(ctx, method, attribute.Int("categoryID", *req.CategoryID))
	defer func() { end(status) }()
	...
	if err != nil {
		status = "error"
		return errorhandler.HandleError[*models.Category](s.logger, err, method, span, zap.Int("category_id", *req.CategoryID))
	}
...
func (s *categoryCommandService) Trash(ctx context.Context, categoryID int) (*models.Category, error) {
	const method = "Trash"

	ctx, span, end, status, logSuccess := s.observability.StartTracingAndLogging(ctx, method, attribute.Int("categoryID", categoryID))
	defer func() { end(status) }()

	category, err := s.categoryCommandRepository.Trash(ctx, categoryID)
	if err != nil {
		status = "error"
		return errorhandler.HandleError[*models.Category](s.logger, err, method, span, zap.Int("category_id", categoryID))
	}
...
func (s *categoryCommandService) Restore(ctx context.Context, categoryID int) (*models.Category, error) {
...
func (s *categoryCommandService) DeletePermanent(ctx context.Context, categoryID int) (bool, error) {
	...
	if err != nil {
		status = "error"
		return errorhandler.HandleError[bool](s.logger, err, method, span, zap.Int("category_id", categoryID))
	}
	...
	if category.ImageCategory != nil && *category.ImageCategory != "" {
		...
		if err != nil {
			if os.IsNotExist(err) {
				s.logger.Debug("Category image file not found, continuing with category deletion", zap.String("image_path", *category.ImageCategory))
			} else {
				status = "error"
				return errorhandler.HandleError[bool](s.logger, category_errors.ErrFailedRemoveImageCategory, method, span, zap.String("image_path", *category.ImageCategory))
			}
		} else {
			s.logger.Debug("Successfully deleted category image", zap.String("image_path", *category.ImageCategory))
		}
	}
