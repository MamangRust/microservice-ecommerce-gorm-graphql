...
func (s *registerService) Register(ctx context.Context, request *requests.RegisterRequest) (*repository.AuthUser, error) {
	...
	if err != nil || role == nil {
		status = "error"
		return sharederrorhandler.HandleError[*repository.AuthUser](s.logger, err, method, span, zap.String("role_name", defaultRoleName))
	}
	...
	if err != nil {
		status = "error"
		return sharederrorhandler.HandleError[*repository.AuthUser](s.logger, err, method, span, zap.Int("user.id", int(newUser.UserID)))
	}
