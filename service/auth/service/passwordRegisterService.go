...
func (s *passwordResetService) ForgotPassword(ctx context.Context, email string) (bool, error) {
	...
	if s.gormDB != nil {
		// GORM transaction: reset token insert + outbox event are atomic
		txErr := s.gormDB.Transaction(func(tx *gorm.DB) error {
			...
			if s.outbox != nil {
				return s.outbox.EnqueueInTx(ctx, NewGormOutboxQuerier(tx), "email-service-topic-auth-forgot-password", strconv.Itoa(int(res.UserID)), payloadBytes)
			}
			return nil
		})
		if txErr != nil {
			status = "error"
			return sharederrorhandler.HandleError[bool](s.logger, txErr, method, span, zap.Int("user.id", int(res.UserID)))
		}
	} else {
