...
func NewCartQueryService(deps *CartQueryServiceDeps) *cartQueryService {
...
func (s *cartQueryService) FindAll(ctx context.Context, req *requests.FindAllCarts) ([]*repository.CartResult, *int, error) {
	...
	if err != nil {
		status = "error"
		return errorhandler.HandlerErrorPagination[[]*repository.CartResult](
			s.logger,
			err,
			method,
			span,
	...
	logSuccess("Successfully fetched all carts",
		zap.Int("totalRecords", totalCount), zap.Int("page", page), zap.Int("pageSize", pageSize))
