...
func (s *productCommandService) Create(ctx context.Context, req *requests.CreateProductRequest) (*models.Product, error) {
	const method = "Create"
	...
	if err != nil {
		status = "error"
		return errorhandler.HandleError[*models.Product](
			s.logger,
			err,
			method,
			span,
...
func (s *productCommandService) Update(ctx context.Context, req *requests.UpdateProductRequest) (*models.Product, error) {
	const method = "Update"
...
func (s *productCommandService) UpdateProductCountStock(ctx context.Context, productID int, stock int) (*models.Product, error) {
...
func (s *productCommandService) AdjustProductStock(ctx context.Context, productID int, delta int, operationID string) (*models.Product, error) {
	...
	if productID <= 0 || delta == 0 || operationID == "" {
		status = "error"
		return errorhandler.HandleError[*models.Product](s.logger, product_errors.ErrFailedCountStock, method, span)
	}

	product, err := s.productRepository.AdjustProductStock(ctx, productID, delta, operationID)
	if err != nil {
		status = "error"
		return errorhandler.HandleError[*models.Product](s.logger, err, method, span)
	}
