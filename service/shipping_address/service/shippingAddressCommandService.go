...
func (s *shippingAddressCommandService) Create(ctx context.Context, request *requests.CreateShippingAddressRequest) (*models.ShippingAddress, error) {
	const method = "Create"
	ctx, span, end, status, logSuccess := s.observability.StartTracingAndLogging(ctx, method,
		attribute.String("alamat", request.Alamat), attribute.String("provinsi", request.Provinsi), attribute.String("kota", request.Kota))
	defer func() { end(status) }()

	shippingAddress, err := s.shippingAddressRepository.Create(ctx, request)
	if err != nil {
		status = "error"
		return errorhandler.HandleError[*models.ShippingAddress](s.logger, shipping_address_errors.ErrFailedCreateShippingAddress, method, span, zap.Error(err))
	}
...
func (s *shippingAddressCommandService) Update(ctx context.Context, request *requests.UpdateShippingAddressRequest) (*models.ShippingAddress, error) {
	const method = "Update"
	ctx, span, end, status, logSuccess := s.observability.StartTracingAndLogging(ctx, method, attribute.Int("shipping_id", *request.ShippingID))
	defer func() { end(status) }()

	shippingAddress, err := s.shippingAddressRepository.Update(ctx, request)
	if err != nil {
		status = "error"
		return errorhandler.HandleError[*models.ShippingAddress](s.logger, shipping_address_errors.ErrFailedUpdateShippingAddress, method, span, zap.Error(err))
	}

	s.cache.InvalidateShippingAddressCache(ctx)
	logSuccess("Successfully updated shipping address", zap.Int("shipping_id", *request.ShippingID))
	return shippingAddress, nil
}

func (s *shippingAddressCommandService) Trash(ctx context.Context, shipping_id int) (*models.ShippingAddress, error) {
	const method = "Trash"
	...
	if err != nil {
		status = "error"
		return errorhandler.HandleError[*models.ShippingAddress](s.logger, err, method, span, zap.Int("shipping_id", shipping_id))
	}

	s.cache.InvalidateShippingAddressCache(ctx)
	logSuccess("Successfully trashed shipping address", zap.Int("shipping_id", shipping_id))
	return shippingAddress, nil
}

func (s *shippingAddressCommandService) Restore(ctx context.Context, shipping_id int) (*models.ShippingAddress, error) {
	...
	s.cache.InvalidateShippingAddressCache(ctx)
	logSuccess("Successfully restored shipping address", zap.Int("shipping_id", shipping_id))
	return shippingAddress, nil
}
...
func (s *shippingAddressCommandService) DeletePermanent(ctx context.Context, shipping_id int) (bool, error) {
	...
	if err != nil {
		status = "error"
		return errorhandler.HandleError[bool](s.logger, err, method, span, zap.Int("shipping_id", shipping_id))
	}

	s.cache.InvalidateShippingAddressCache(ctx)
	logSuccess("Successfully permanently deleted shipping address", zap.Int("shipping_id", shipping_id))
	return success, nil
}
...
func (s *shippingAddressCommandService) DeleteShippingAddressByOrderPermanent(ctx context.Context, order_id int) (bool, error) {
	...
	if err != nil {
		status = "error"
		return errorhandler.HandleError[bool](s.logger, shipping_address_errors.ErrFailedDeleteShippingAddressPermanent, method, span, zap.Error(err), zap.Int("order_id", order_id))
	}

	s.cache.InvalidateShippingAddressCache(ctx)
	logSuccess("Successfully permanently deleted shipping addresses by order", zap.Int("order_id", order_id))
	return success, nil
}
...
func (s *shippingAddressCommandService) RestoreAll(ctx context.Context) (bool, error) {
	...
	if err != nil {
		status = "error"
		return errorhandler.HandleError[bool](s.logger, shipping_address_errors.ErrFailedRestoreAllShippingAddresses, method, span)
	}
