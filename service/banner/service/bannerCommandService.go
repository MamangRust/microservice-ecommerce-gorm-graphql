...
func (s *bannerCommandService) Create(ctx context.Context, req *requests.CreateBannerRequest) (*models.Banner, error) {
	const method = "Create"
	...
	if err != nil {
		status = "error"
		return errorhandler.HandleError[*models.Banner](
			s.logger, banner_errors.ErrFailedCreateBanner.WithInternal(err), method, span, zap.Any("request", req),
		)
	}
...
func (s *bannerCommandService) Update(ctx context.Context, req *requests.UpdateBannerRequest) (*models.Banner, error) {
	const method = "Update"

	ctx, span, end, status, logSuccess := s.observability.StartTracingAndLogging(ctx, method, attribute.Int("bannerID", *req.BannerID))
	defer func() { end(status) }()
	...
	if err != nil {
		status = "error"
		return errorhandler.HandleError[*models.Banner](
			s.logger, banner_errors.ErrFailedUpdateBanner.WithInternal(err), method, span, zap.Int("banner_id", *req.BannerID),
		)
	}
...
func (s *bannerCommandService) Trash(ctx context.Context, bannerID int) (*models.Banner, error) {
	const method = "Trash"
	...
	if err != nil {
		status = "error"
		return errorhandler.HandleError[*models.Banner](s.logger, err, method, span, zap.Int("banner_id", bannerID))
	}
...
func (s *bannerCommandService) Restore(ctx context.Context, bannerID int) (*models.Banner, error) {
...
func (s *bannerCommandService) DeletePermanent(ctx context.Context, bannerID int) (bool, error) {
	...
	if err != nil {
		status = "error"
		return errorhandler.HandleError[bool](s.logger, err, method, span, zap.Int("banner_id", bannerID))
	}
...
func (s *bannerCommandService) RestoreAll(ctx context.Context) (bool, error) {
	...
	if err != nil {
		status = "error"
		return errorhandler.HandleError[bool](s.logger, banner_errors.ErrFailedRestoreAllBanners.WithInternal(err), method, span)
	}
