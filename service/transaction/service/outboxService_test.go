...
type fakeOutboxRepository struct {
	pending      []*repository.OutboxEventResult
	deliveredIDs map[int64]bool
	deadIDs      map[int64]bool
	failedIDs    map[int64]time.Time
}

func (f *fakeOutboxRepository) Create(ctx context.Context, topic, key string, payload []byte) (*repository.OutboxEventResult, error) {
	ev := &repository.OutboxEventResult{
...
func (f *fakeOutboxRepository) CreateInTx(ctx context.Context, tx *gorm.DB, topic, key string, payload []byte) (*repository.OutboxEventResult, error) {
	return f.Create(ctx, topic, key, payload)
}

func (f *fakeOutboxRepository) GetPending(ctx context.Context, limit int) ([]*repository.OutboxEventResult, error) {
	return f.pending, nil
}

func (f *fakeOutboxRepository) Claim(ctx context.Context, limit int, leaseUntil time.Time) ([]*repository.OutboxEventResult, error) {
	claimed := f.pending
...
func (f *fakeOutboxRepository) MarkDelivered(ctx context.Context, outboxID int64) (*repository.OutboxEventResult, error) {
	if f.deliveredIDs == nil {
		f.deliveredIDs = map[int64]bool{}
	}
	f.deliveredIDs[outboxID] = true
	return &repository.OutboxEventResult{OutboxID: outboxID, Status: "delivered"}, nil
}

func (f *fakeOutboxRepository) MarkFailed(ctx context.Context, outboxID int64, nextAttemptAt time.Time) (*repository.OutboxEventResult, error) {
	if f.failedIDs == nil {
		f.failedIDs = map[int64]time.Time{}
	}
	f.failedIDs[outboxID] = nextAttemptAt
	return &repository.OutboxEventResult{OutboxID: outboxID, Status: "pending"}, nil
}

func (f *fakeOutboxRepository) MarkDead(ctx context.Context, outboxID int64) (*repository.OutboxEventResult, error) {
	if f.deadIDs == nil {
		f.deadIDs = map[int64]bool{}
	}
	f.deadIDs[outboxID] = true
	return &repository.OutboxEventResult{OutboxID: outboxID, Status: "dead"}, nil
}
